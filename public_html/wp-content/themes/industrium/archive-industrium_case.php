...
$columns_number = industrium_get_theme_mod('case_studies_archive_columns_number');
$posts_per_page = industrium_get_theme_mod('case_studies_archive_posts_per_page');
                    ...
                    <div class="archive-listing-wrapper case-study-listing-wrapper case-study-grid-listing<?php echo ( isset($columns_number) && !empty($columns_number) ? ' columns-' . esc_attr($columns_number) : '' ); ?>">
                        <?php
                            while( $query->have_posts() ){
                                $query->the_post();
                                get_template_part('content', 'industrium_case');
                            };
                            wp_reset_postdata();
                        ?>
                    </div>
